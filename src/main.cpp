...
int main(int argc, char **argv) {
    ...
    vector<Tonality*> tonalities = {Cminor, Ebmajor, Cminor};
    vector<int> modulationTypes = {PERFECT_CADENCE_MODULATION, SECONDARY_DOMINANT_MODULATION};
    vector<int> modulationStarts = {12, 22};
    vector<int> modulationEnds = {13, 23};
    ...
    /// todo split the generated sections into phrases and call diatony for each phrase
    ...
    for (int cp = 0; cp < tonalities.size(); cp++) {
        ...
        auto curr_sec_notes = intVarArray_to_int_vector(voicing->getFullVoicing());
        for (auto n: curr_sec_notes)
            all_notes.push_back(n);
    }
    write_midifile("out/MidiFiles/piece", all_notes);
